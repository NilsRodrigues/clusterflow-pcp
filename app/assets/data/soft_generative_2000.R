...
exportDir <- "../../../../paper/graphics/soft_generated/2000/" #"./"
exportPDF <- T
exportSVG <- T
exportCSV <- T
dotSize <- 0.3
...
renderPlot <- function(plotFunction, plotName) {
  ...
  if (brushing) {
    plotName <- paste(plotName, "_brushed", sep="")
  }
...
if (brushing) {
  ...
  clusters <- cluster(dim1="D1", dim2="D2", epsMin=0.04, epsMax=0.08, ptsMin=1, ptsMax=5)
}
...
d3b_length <- ceiling(0.28 * n)
d3d_length <- ceiling(0.02 * n)
d3c_length <- n - d3a_length - d3b_length - d3d_length
...
# generate values for cluster connection
d3c <- rnorm(n=d3c_length * 2, mean=0.2, sd=sd3)
# limit range (0..1)
d3c <- d3c[d3c >= 0 & d3c <= 1]
d3c <- head(d3c, d3c_length)
...
# generate values for cluster connection
d3d <- rnorm(n=d3d_length * 2, mean=0.4, sd=sd3*2.5)
# limit range (0..1)
d3d <- d3d[d3d >= 0 & d3d <= 1]
d3d <- head(d3d, d3d_length)


d3 <- c(d3a,d3c,d3b, d3d)

# manual tweaking
tweak <- c(1965,1975,1986) # cluster in lower center
tweak <- c(tweak, 1761,1841,1943) # noise on the right
d3[tweak] <- d3[tweak] - 0.05

tweak <- c(1064,1271,1312,1323) # cluster in lower left
tweak <- c(tweak, 225, 237, 491, 1171) # noise in the center
d3[tweak] <- d3[tweak] + 0.05

tweak <- c(1016) # noise on the left
d3[tweak] <- d3[tweak] + 0.5

allData$D3 <- d3

#clusters <- cluster(dim1="D2", dim2="D3", epsMin=0.092, ptsMin=1)
if (brushing) {
  clusters <- cluster(dim1="D2", dim2="D3", epsMin=0.05, ptsMin=2)
} else {
  clusters <- cluster(dim1="D2", dim2="D3", epsMin=0.04, epsMax=0.046, ptsMin=1, ptsMax=10)
}
...
if (brushing) {
  clusters <- cluster(dim1="D3", dim2="D4", epsMin=0.05, ptsMin=5)
} else {
  clusters <- cluster(dim1="D3", dim2="D4", epsMin=0.04, epsMax=0.06, ptsMin=5, ptsMax=10)
}
...
if (brushing) {
  ...
  # set all weights to a low value so the points are in a lower layer when rendering (also removes original fuzzyness)
  for (dim in clusterDims)
    allData[!is.na(allData[,dim]), dim] <- 0.15
...
if (exportCSV) {
  ...
  if (brushing) {
    outputFileName <- paste(currentFileName, "_brushed.csv", sep="")
  } else {
    outputFileName <- paste(currentFileName, ".csv", sep="")
  }

  write.csv(allData, file=outputFileName, row.names=FALSE, na="")
}
