...
clusterScatterPlot <- function(data) {
  ...
  for(i in 1:length(clusterColumns)) {
    ...
    if (hasNoise && i == noiseIndex)
      ...
      col <- colorsToUse[i]
    points(x=x, y=y, pch=16, cex=dotSize, col=col)
#    points(x=x, y=y, pch=16, cex=dotSize*(length(clusterColumns)-i+1), col=col)
  }
}
...
# manual tweaking
#d3[c(2883, 3312)] <- d3[c(2883, 3312)]+0.5
tweak <- c(1542, 2879, 2883, 3138, 3312, 3390, 3404, 4906, 4911, 4917, 4920, 4922, 4946, 4948, 4952, 4959, 4971, 4973, 4986)
d3[tweak] <- d3[tweak] + 0.4

allData$D3 <- d3

if (brushing) {
  clusters <- cluster(dim1="D2", dim2="D3", epsMin=0.046, ptsMin=1)
} else {
...
if (brushing) {
  clusters <- cluster(dim1="D3", dim2="D4", epsMin=0.06, ptsMin=5)
} else {
